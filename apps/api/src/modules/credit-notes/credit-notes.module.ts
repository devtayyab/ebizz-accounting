...
import type { CreditNote } from "@ebizz/shared";
...
class CreditNotesService {
  ...
  async list(companyId: string): Promise<CreditNote[]> {
    const { data, error } = await this.db.from("credit_notes").select("*").eq("company_id", companyId).order("note_date", { ascending: false });
    if (error) throw new BadRequestException(pgMessage(error));
    return (data ?? []) as CreditNote[];
  }

  async get(id: string): Promise<CreditNote> {
    const { data, error } = await this.db.from("credit_notes").select("*, lines:credit_note_lines(*)").eq("id", id).maybeSingle();
    if (error) throw new BadRequestException(pgMessage(error));
    if (!data) throw new NotFoundException("Credit note not found");
    return data as CreditNote;
  }
  ...
  async create(dto: CreateCreditNoteDto): Promise<CreditNote> {
    ...
    const number = await nextDocNumber(this.db, "credit_notes", "note_number", dto.company_id, "CN-");
    ...
    const { data: note, error } = await this.db.from("credit_notes").insert({
      organization_id, company_id: dto.company_id, customer_id: dto.customer_id,
      invoice_id: dto.invoice_id ?? null, location_id: dto.location_id ?? null,
    ...
    const { error: le } = await this.db.from("credit_note_lines").insert(computed.map((c) => ({ ...c, note_id: noteId })));
    if (le) throw new BadRequestException(pgMessage(le));
    return this.get(noteId);
  }

  async update(id: string, dto: CreateCreditNoteDto): Promise<CreditNote> {
    const existing = await this.get(id);
    if (existing.status !== "draft") throw new BadRequestException("Only draft credit notes can be edited");
    await this.db.from("credit_note_lines").delete().eq("note_id", id);
    const computed = dto.lines.map((l, i) => {
      const c = computeLine(Number(l.quantity), Number(l.unit_price), Number(l.tax_rate ?? 0));
      return {
    ...
    const currency = dto.currency || base_currency;
    const fx_rate = currency === base_currency ? "1" : (dto.fx_rate && Number(dto.fx_rate) > 0 ? dto.fx_rate : "1");
    const { error } = await this.db.from("credit_notes").update({
      customer_id: dto.customer_id, invoice_id: dto.invoice_id ?? null, location_id: dto.location_id ?? null,
      restock: dto.restock ?? true, currency, fx_rate, subtotal, tax_total, total: sum([subtotal, tax_total]),
    }).eq("id", id);
    if (error) throw new BadRequestException(pgMessage(error));
    const { error: le } = await this.db.from("credit_note_lines").insert(computed);
    if (le) throw new BadRequestException(pgMessage(le));
    return this.get(id);
  }

  async post(id: string): Promise<CreditNote> {
    const { error } = await this.db.rpc("post_credit_note", { p_id: id });
    if (error) throw new BadRequestException(pgMessage(error));
    return this.get(id);
  }

  async remove(id: string): Promise<void> {
    const { error } = await this.db.from("credit_notes").delete().eq("id", id).eq("status", "draft");
    if (error) throw new BadRequestException(pgMessage(error));
  }
}
...
class CreditNotesController {
  ...
  @Post() create(@Body() dto: CreateCreditNoteDto) { return this.svc.create(dto); }
  @Patch(":id") update(@Param("id", ParseUUIDPipe) id: string, @Body() dto: CreateCreditNoteDto) { return this.svc.update(id, dto); }
...
@Module({ controllers: [CreditNotesController], providers: [CreditNotesService, AuthGuard] })
